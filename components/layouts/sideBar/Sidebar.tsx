import Link from "next/link";
import Image from "next/image";
// import MobileCTA from '../ui/MobileCTA'

const EmailIcon = () => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <rect x="2" y="4" width="20" height="16" rx="2" />
        <path d="m2 7 10 7 10-7" />
    </svg>
)

const GitHubIcon = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
)

const LinkedInIcon = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
)

const statusItems = [
    { label: 'STATUS', value: 'ONLINE', mod: 'online' },
    { label: 'LOCATION', value: 'MOROCCO', mod: null },
    { label: 'TIMEZONE', value: 'UTC+1', mod: null },
    { label: 'AVAILABLE', value: 'OPEN', mod: 'open' },
]

function Sidebar() {
    return (
        <aside className="sidebar">
            <Link href="/" className="sidebar__logo">
                <span>ECHCHARQUI</span>
            </Link>

            <div className="sidebar__body">
                <div className="sidebar__profile">
                    <div className="profile-frame">
                        <span className="profile-frame__corner profile-frame__corner--tl" />
                        <span className="profile-frame__corner profile-frame__corner--tr" />
                        <span className="profile-frame__corner profile-frame__corner--bl" />
                        <span className="profile-frame__corner profile-frame__corner--br" />
                        <div className="profile-frame__inner">
                            <Image src="/echcharqui.gif" width={220} height={220} loading="eager" alt="Echcharqui" className="profile-frame__img" />
                        </div>
                    </div>
                </div>

                <div className="sidebar__info">
                    <nav className="sidebar__socials" aria-label="Social links">
                        <a
                            href="mailto:astrosoft.digital.agency@gmail.com"
                            target="_blank"
                            rel="noopener noreferrer"
                            aria-label="Email"
                            className="sidebar__social-link"
                        >
                            <EmailIcon />
                        </a>
                        <a
                            href="https://github.com/ECHCHARQUI"
                            target="_blank"
                            rel="noopener noreferrer"
                            aria-label="GitHub"
                            className="sidebar__social-link"
                        >
                            <GitHubIcon />
                        </a>
                        <a
                            href="https://linkedin.com/in/ECHCHARQUI"
                            target="_blank"
                            rel="noopener noreferrer"
                            aria-label="LinkedIn"
                            className="sidebar__social-link"
                        >
                            <LinkedInIcon />
                        </a>
                    </nav>

                    <dl className="sidebar__status">
                        {statusItems.map(({ label, value, mod }) => (
                            <div key={label} className="sidebar__status-row">
                                <dt className="sidebar__status-label">{label}</dt>
                                <dd className={`sidebar__status-value${mod ? ` sidebar__status-value--${mod}` : ''}`}>
                                    {value}
                                </dd>
                            </div>
                        ))}
                    </dl>
                </div>
            </div>

            <footer className="sidebar__footer">
                <p>FULL-STACK SOFTWARE ENGINEER • MOROCCO</p>
            </footer>

            {/* <MobileCTA /> */}
        </aside>
    )
}

export default Sidebar
